    ...
    void Create_Wheel (string direction, int number)
		{
			...
			// Drive_Wheel_CS
			TrackWheel driveScript = wheelObject.AddComponent <TrackWheel> ();
			driveScript.This_Rigidbody = rigidbody;
			driveScript.Is_Left = (direction == "L");
            driveScript.controlScript=thisTransform.parent.gameObject.GetComponent< Engine > ();
			...
			driveScript.This_Transform = wheelObject.transform;
			driveScript.Is_Left = (direction == "L");
			driveScript.Initial_Pos_Y = wheelObject.transform.localPosition.y;
			driveScript.Initial_Angles = wheelObject.transform.localEulerAngles;
            driveScript.Initial_Locoal_Pos=wheelObject.transform.localPosition;
			// Set Layer
			// wheelObject.layer = 9;
		}
